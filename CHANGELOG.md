...
### Added
- **Ambient selection toggle**: New `TRELLO_MCP_AMBIENT_SELECTION=on|off` environment variable controls whether the server keeps an active board/workspace across calls. Unset, it follows the transport: **on** for stdio (default, unchanged), **off** for HTTP (`TRELLO_MCP_TRANSPORT=http`). One `TrelloClient` is shared by every MCP session, so under HTTP one caller's `set_active_board` would silently retarget another caller's next unqualified call, and the selection was persisted to `~/.trello-mcp/config.json`, leaking across restarts. An invalid value fails at startup. With ambient selection off: `set_active_board`, `set_active_workspace` and `get_active_board_info` are not registered (absent from `tools/list`); board resolution is explicit `boardId` then `TRELLO_BOARD_ID`, with no active board; workspace targeting is explicit-only, so `create_board` under `TRELLO_ALLOWED_WORKSPACES` requires an explicit `idOrganization`; `~/.trello-mcp/config.json` is never read or written; board-scoped health checks report not-applicable instead of permanently DEGRADED, and `perform_system_repair` reports no repairs available. `TRELLO_BOARD_ID` works in both modes and `list_boards_in_workspace` is unaffected. Transport configuration is now read before the Trello client is constructed, so an HTTP process no longer reads the persisted board off disk before it knows which transport it is running. Programmatic construction can set the equivalent optional `ambientSelection` field on `TrelloConfig` (defaults to `true`).
- **Card Un-archiving**: `unarchive_card(cardId)` - Return a card from the archive to the list it was archived from; the reverse of `archive_card`
...
### Changed
- **BREAKING**: Dropped the `boardId` input from ten tools that declared it but never sent it to Trello: `get_cards_by_list_id`, `add_card_to_list`, `update_card_details`, `archive_card`, `unarchive_card`, `archive_list`, `attach_image_to_card`, `attach_file_to_card`, `attach_data_to_card`, `attach_image_data_to_card`. Card, list and attachment IDs are globally unique, so these calls always reached the right object whatever was passed - but the parameter *read* as a scoping control, so a caller passing it believed it had switched board context when it had not. Callers must stop passing `boardId` to these ten tools. Tools that genuinely scope by board keep it: `get_lists`, `get_recent_activity`, `move_card`, `add_list_to_board`, `get_card_by_short`, `get_checklist_items`, `add_checklist_item`, `find_checklist_items_by_description`, `get_acceptance_criteria`, `get_checklist_by_name`, `get_board_members`, `get_board_labels`, `create_label`, `get_board_custom_fields`
- **BREAKING**: Nine `TrelloClient` methods lost their leading `boardId` parameter to match: `addCard`, `updateCard`, `archiveCard`, `unarchiveCard`, `archiveList`, `attachImageToCard`, `attachDataToCard`, `attachImageDataToCard`, `attachFileToCard`. For example, `client.addCard(undefined, { listId, name })` becomes `client.addCard({ listId, name })`, and `client.archiveCard(undefined, cardId)` becomes `client.archiveCard(cardId)`
- **Unified board resolution**: Every board-scoped client method now resolves its target board through a single resolver - explicit `boardId` argument, then the active board, then the `TRELLO_BOARD_ID` default - replacing thirteen call sites that each derived it inline in one of three different shapes

### Fixed
- **BEHAVIOUR CHANGE**: `move_card` no longer ignores the active board. It resolved `boardId || TRELLO_BOARD_ID`, skipping the active board entirely, so after `set_active_board(B)` a `move_card` with no explicit `boardId` sent the *default* board alongside a list living on board B - a contradictory request that either errored or moved the card off B. It now honours the active board like every other board-scoped call
